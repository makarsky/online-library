...
public class CheckSessionFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        //String errorMessage = Messages.getMessage(wrappedRequest.getLocale().getLanguage(), "error.session_timeout");
        try {
            if (session == null || session.isNew()) {
                wrappedResponse.setCharacterEncoding("UTF-8");
                wrappedRequest.setCharacterEncoding("UTF-8");
                wrappedResponse.sendRedirect(wrappedRequest.getContextPath() + "/index.jsp?errorMessage=error.session_timeout");
            } else {
